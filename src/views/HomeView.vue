<template>
  <div class="home-view">
    ...
    <section class="center-panel">
      <ElCard class="center-card fullspan video-card">
        <div class="video-container">
          <!-- 视频 / 数字人切换显示 -->
          <div class="video-switch-wrapper">

            <!-- 切换按钮 -->
            <div class="video-switch-btn">
              <ElButton size="small" type="primary" @click="centerMode = centerMode === 'video' ? 'avatar' : 'video'">
                {{ centerMode === 'video' ? '切换到数字人' : '切换到视频' }}
              </ElButton>
            </div>

            <!-- 视频 -->
            <video v-if="centerMode === 'video'" ref="satVideoRef" class="video" :src="assets.satelliteVideo" controls
              autoplay muted loop playsinline></video>

            <!-- 数字人 -->
            <div style="display: flex;align-items: flex-end;" v-else>
              <ChatBox></ChatBox>
              <DigitalHumanChat />
            </div>
          </div>
        ...
        <div class="video-info">
          <div class="info-left">
            <VChart :option="speedOption" class="speed-chart" />
          </div>

          <div class="info-right">
            <div class="carousel-body" :class="{ 'overlay-debug': overlayDebug }">
              <ElTable :data="carouselRows" class="carousel-table" :highlight-current-row="false" style="width: 100%">
                <ElTableColumn prop="time" label="时间" width="100" />
                <ElTableColumn prop="source" label="来源" width="100" />
                <ElTableColumn prop="risk" label="风险" width="60" />
    ...
    <section class="side-panel">
      <ElCard class="data-card">
        <RealtimeChart title="滑坡预测" unit="kPa" :min="40" :max="200" color="#f97316" chartType="line" />
      </ElCard>

      <ElCard class="data-card">
        <RealtimeChart title="洪水预测" unit="mm" :min="0" :max="200" color="#06b6d4" chartType="bar" />
      </ElCard>

      <ElCard class="data-card">
        <RealtimeChart title="泥石流预测" unit="%" :min="0" :max="100" color="#ef4444" chartType="stack" />
      </ElCard>
    </section>
  </div>
</template>
...
import DigitalHumanChat from '@/components/DigitalHumanChat.vue'
import ChatBox from '@/components/ChatBox.vue'

const centerMode = ref('video')
const satVideoRef = ref(null)

watch(centerMode, (val) => {
  if (val === 'avatar') {
    satVideoRef.value?.pause()
  } else {
    satVideoRef.value?.play?.().catch(() => { })
  }
})


// register required components
use([LineChart, BarChart, , PieChart, GridComponent, TooltipComponent, TitleComponent, LegendComponent, CanvasRenderer])
...
function generateSpeedOption(titleLabel, dataArr, timeArr, yUnit) {
  return {
    ...
    title: { text: titleLabel, textStyle: { color: '#fff', fontSize: 15 }, left: 'center', top: 2 },
    xAxis: {
      type: 'category',
      data: timeArr,
...
.video-card {
  padding: 0;
  /* video container will handle spacing */
  position: relative;
  /* allow overlay positioned inside card */
  display: flex;
  flex-direction: column;
}
...
.video-container {
  display: block;
  flex: 0 0 60%;
  /* video takes ~60% of card height */
  background-image: url(../assets/lp041/chat5.jpg);
  /* opacity: 0.9; */
}
...
.video-info {
  ...
  z-index: 2;
  flex: 1 1 auto;
  /* fill remaining height under video */
  align-items: stretch;
}
...
.data-card {
  ...
  flex-direction: column;
  min-height: 200px;
  /* allow proper flex child sizing inside grid */
}
...
:deep(.carousel-table td.el-table__cell, .el-table th.el-table__cell.is-leaf) {
  border-bottom: none;
}
