...
function updateOption() {
  option.value = {
    ...
    title: { text: '降雨量 (mm)', textStyle: { color: '#fff', fontSize: 12 }, left: 'center', top: 6 },
    grid: { left: '6%', right: '6%', top: '18%', bottom: '8%', containLabel: true },
    xAxis: { type: 'category', data: time.value, axisLabel: { color: '#bbb', fontSize: 10 } },
    yAxis: { type: 'value', name: 'mm', axisLabel: { color: '#bbb', fontSize: 10 } },
    tooltip: { trigger: 'axis' },
    series: [{ type: 'line', data: data.value, smooth: true, lineStyle: { color: '#4ae88a' }, areaStyle: { color: new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:'rgba(74,232,138,0.28)'},{offset:1,color:'rgba(74,232,138,0)'}]) }, showSymbol:false }]
  }
}
