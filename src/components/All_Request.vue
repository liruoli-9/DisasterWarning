    ...
    <el-container>
      ...
      <el-main>
        <el-table :data="tableData1" style="width: 100%" @selection-change="handleSelectionChange">
          ...
          <el-table-column property="request_nickname" label="申请人" width="120" />
          <el-table-column property="request_city" label="申请城市" width="120" />
          ...
          <el-table-column label="操作">
            <template #default="{ row }">
              <el-button :disabled="row.request_state == '已分配' || row.request_state == '拒绝分配'" size="small" type="danger"
                @click="confirm(row)">
                确定分配
              </el-button>
              <el-button :disabled="row.request_state == '已分配' || row.request_state == '拒绝分配'" size="small" type="danger"
...
import { get_allRequest } from "@/api/get_allRequest.js";
...
try {
  let data = await item_allocation(
    {
      "request_Id": row.request_Id,
      "allocation_state": "已分配",
      "item_request_number": row.item_request_number,
  ...
  if (data.code == 1) {
    ...
    ElMessage({
      type: "error",
      message: data.msg || "分配失败"
    });
  }
} catch (error) {
    const errorMessage = error.message.includes("Insufficient item number")
      ? "物品数量不足"
...
async function cancel(row) {
  ...
  let data = await item_allocation({ "request_Id": row.request_Id, "allocation_state": "拒绝分配" })
  // let data1=await delete_request(row.request_Id)
  console.log(data);
  if (data.code == 1) {
    ElMessage({
      type: "success",
...
onMounted(async () => {
  ...
  let data = await get_allRequest(data1.value.userId);
