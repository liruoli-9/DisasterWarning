...
function genInitial(len = 30, min = 50, max = 150) {
  return Array.from({ length: len }, () => Math.random() * (max - min) + min)
}

const time = ref(createTimeArray(props.len))
const data = ref(genInitial(props.len, 50, 150))
const option = ref({})

function updateOption() {
  option.value = {
    backgroundColor: 'transparent',
    title: { text: '孔隙水压力 (kPa)', textStyle: { color: '#fff', fontSize: 12 }, left: 'center', top: 6 },
    grid: { left: '6%', right: '6%', top: '18%', bottom: '8%', containLabel: true },
    xAxis: { type: 'category', data: time.value, axisLabel: { color: '#bbb', fontSize: 10 } },
    yAxis: { type: 'value', name: 'kPa', axisLabel: { color: '#bbb', fontSize: 10 } },
    tooltip: { trigger: 'axis' },
    series: [{ type: 'line', data: data.value, smooth: true, lineStyle: { color: '#4a9eff' }, areaStyle: { color: new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:'rgba(74,158,255,0.28)'},{offset:1,color:'rgba(74,158,255,0)'}]) }, showSymbol:false }]
  }
}
...
function tick() {
  data.value.shift()
  data.value.push(Math.random() * 100 + 50)
  const now = new Date()
  time.value.shift()
